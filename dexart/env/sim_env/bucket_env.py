...
class BucketEnv(BaseSimulationEnv):
    ...
    def render(self, mode="human"):
        ...
        if mode == 'human':
            ...
            self.cameras['bucket_viz'].take_picture()
            rgba = self.cameras['bucket_viz'].get_float_texture('Color')
            rgba_img = (rgba * 255).clip(0, 255).astype("uint8")
            rgba_pil = Image.fromarray(rgba_img)
            # rgba_pil.save(f'/home/luoyu/Workspace/zhengkx/dexart-release/render/bucket_{self.render_step}.png')
            self.render_step += 1
            return rgba_img
        else:
